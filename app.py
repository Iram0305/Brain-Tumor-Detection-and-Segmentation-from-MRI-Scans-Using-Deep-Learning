...
# UI v4.3: High-Legibility Dashboard · Restored Image Overlay Text
...
/* hide streamlit default UI chrome - FIXED to allow sidebar toggle and menu */
...
def draw_highlight(original: Image.Image, bbox: tuple, tumor_mask: np.ndarray) -> Image.Image:
    ...
    # --- FIX: Restored the text drawing for the overlay image! ---
    bh = 30
    bx, by = bbox[0], max(0, bbox[1] - bh - 5)
    draw.rectangle([bx, by, bx+148, by+bh], fill=(200, 25, 45)) 
    draw.text((bx+8, by+8), "ANOMALY DETECTED", fill=(255, 255, 255))
